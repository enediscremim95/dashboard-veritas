"""
All Clinique Dashboard — geração + deploy automático.
Roda: python generate_all_clinique.py

Clínica Estética · Portugal
Conversão: lead
Moeda: EUR (€)
NOTA: Veritas assumiu gestão em 17/04/2026
"""
...
#  CONFIG
# ══════════════════════════════════════════════════════════════════════
CLIENTE_NOME  = "All Clinique"
CLIENTE_SUB   = "Clinica Estetica · Portugal"
CF_SUBFOLDER  = "all-clinique"

HAS_GOOGLE    = True
HAS_ROAS      = False

META_ACT      = "act_651338617661638"
SHEET_ID      = "1gQUQNlQ6I86hrRrJY1JWG64YjvX_u8Ys2U5xH47dJUc"
CLIENTE_SLUG  = "All Clinique"
...
<div class="card card-resumo p-5 mb-4">
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
    ...
    <div><p class="kpi-label">CPA Medio</p><p class="kpi-val text-white" id="tot-cpa">—</p><p class="kpi-sub">custo por lead</p></div>
...
<div class="card card-google p-5 mb-4">
  ...
  <div class="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
    ...
    <div><p class="kpi-label">CPA</p><p class="kpi-val text-white" id="g-cpa">—</p><p class="kpi-sub">custo por lead</p></div>
    <div><p class="kpi-label">Taxa de Conv.</p><p class="kpi-val text-white" id="g-taxa">—</p><p class="kpi-sub">cliques → lead</p></div>
  </div>
...
function update(from,to){{
  ...
  set('google-note', hasG ? '' : 'Sem dados Google no periodo selecionado (campanha pausada).');
  ...
  camps.forEach(c=>{{
    ...
    mbody.innerHTML+=`<tr ${{rowClick}}>
      ...
      <td class="py-3 pr-4 text-right text-gray-300">{CURRENCY_SYM} ${{c.gasto.toFixed(2)}}</td>
