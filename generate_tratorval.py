...
O script:
  1. Puxa 90 dias do Meta Ads via Graph API
  2. Lê a planilha Google Ads (pública, CSV)
  3. Gera o HTML do dashboard
  4. Faz o deploy no Cloudflare Pages

Nenhum dado fica hardcoded — tudo é buscado na hora.
"""
import json, os, io, zipfile, csv, re, requests, hashlib
...
def meta_get(fields, since, until, extra=""):
    ...
    while True:
        ...
        if not nxt or not d.get("paging", {}).get("next"):
            break
    return rows
...
# Dados principais (90 dias)
raw_main = meta_get(GRAPH_FIELDS, since_str, today_str)

# Dados de vídeo (últimos 30 dias — API só entrega onde há vídeo)
...
for row in sorted(raw_main, key=lambda x: x["date_start"]):
    ...
    conv = action_val(row.get("actions"), "onsite_conversion.total_messaging_connection")
...
# ── 2. GOOGLE ADS — planilha pública (CSV) ────────────────────────────────────
print("Lendo planilha Google Ads...")
csv_url = f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/gviz/tq?tqx=out:csv&sheet=Tendencia%20Diaria"
...
# Parse da aba "Tendencia Diaria"
reader = csv.reader(io.StringIO(resp.text))
rows_g = list(reader)

# Encontra cabeçalho com "Data"
header_idx = None
for i, row in enumerate(rows_g):
    if row and "Data" in row[0]:
        header_idx = i
        break
...
if GOOGLE:
    google_last = GOOGLE[-1][0]
print(f"  Google: {len(GOOGLE)} dias  |  ultimo: {google_last}")

# ── 3. CAMPANHAS GOOGLE (aba Campanhas) ───────────────────────────────────────
csv_camp = f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/gviz/tq?tqx=out:csv&sheet=Campanhas"
resp_c = requests.get(csv_camp, timeout=20)
resp_c.encoding = "utf-8"
reader_c = csv.reader(io.StringIO(resp_c.text))
rows_c = list(reader_c)

h_idx = None
for i, row in enumerate(rows_c):
    if row and "Campanha" in row[0] and "Status" in (row[1] if len(row) > 1 else ""):
        h_idx = i
        break

CAMPANHAS = []
_NOISE_BRACKETS = {"site antigo", "site", "antigo", "novo teste", "teste"}

def clean_google_camp(raw):
    """Extrai nome legível do nome bruto de campanha Google Ads."""
    brackets = re.findall(r'\[([^\]]+)\]', raw)
    useful = [b.strip() for b in brackets if b.strip().lower() not in _NOISE_BRACKETS]
    if useful:
        return " / ".join(useful)
    # fallback: remove brackets, datas e prefixo Tratorval
    n = re.sub(r'\[.*?\]|\d{2}/\d{2}/\d{4}|#\d+', '', raw)
    n = re.sub(r'\s{2,}', ' ', n).strip(' -').strip()
    n = re.sub(r'^tratorval\s*[-–\s]*', '', n, flags=re.IGNORECASE).strip(' -').strip()
    return n or raw[:35]
...
if h_idx is not None:
    for row in rows_c[h_idx + 1:]:
        ...
        try:
            ...
            if status not in ("ativa", "ativo", "enabled", "active"):
                continue
            ...
            # Cor pelo CPA
            cor = "green" if (conv > 0 and cpa < 12) else "yellow" if (conv > 0 and cpa < 25) else "red" if conv > 0 else "yellow"
            # Nome legível
            nome = clean_google_camp(nome_raw)
...
# ── 4. CAMPANHAS META (API) ────────────────────────────────────────────────────
print("Puxando campanhas Meta...")
camp_url = f"{META_API}/{META_ACT}/campaigns"
camp_params = {
...
camp_r = requests.get(camp_url, params=camp_params, timeout=20).json()
camp_list = camp_r.get("data", [])
...
for camp in camp_list:
    ...
    data = ci.get("data", [{}])
    if not data or float(data[0].get("spend", 0)) == 0:
        continue
    d0   = data[0]
    ...
    conv     = action_val(d0.get("actions"), "onsite_conversion.total_messaging_connection")
    cpa      = round(gasto / conv, 2) if conv > 0 else 0
    cor      = "green" if conv > 0 and cpa < 8 else "yellow" if conv > 0 and cpa < 15 else "red" if conv > 0 else "blue"
    # Nome legível
    ...
    desc_map = {
        "RMKT": "Quem ja interagiu com a Tratorval",
        "REMARKETING": "Quem ja interagiu com a Tratorval",
        "CONVERSAO": "Objetivo mensagem - topo de funil",
        "CONVERSÃO": "Objetivo mensagem - topo de funil",
        "DISTRIBUICAO": "Alcance para publico novo - trafego",
        "DISTRIBUIÇÃO": "Alcance para publico novo - trafego",
    }
...
# ── 5. DATAS E JSON ───────────────────────────────────────────────────────────
meta_last_fmt   = f"{meta_last[8:10]}/{meta_last[5:7]}/{meta_last[:4]}" if meta_last else "—"
google_last_fmt = f"{google_last[8:10]}/{google_last[5:7]}/{google_last[:4]}" if google_last else "—"
meta_first      = META[0][0] if META else since_str
google_first    = GOOGLE[0][0] if GOOGLE else ""
min_date        = min(meta_first, google_first) if google_first else meta_first
max_date        = meta_last if meta_last else today_str
month_label     = f"{today.replace(day=1).strftime('%d/%m')} — {today_str[8:10]}/{today_str[5:7]}"
camp_label      = f"{month_start[8:10]}/{month_start[5:7]} — {today_str[8:10]}/{today_str[5:7]}"
gcampmonth_start = (GOOGLE[0][0] if GOOGLE else month_start)
gcampmonth_label = f"{gcampmonth_start[8:10]}/{gcampmonth_start[5:7]} — {google_last[8:10]}/{google_last[5:7]}" if google_last else "—"
...
# Ícone Google Ads colorido (SVG inline — azul/amarelo/verde)
...
<title>Tratorval — Painel de Marketing</title>
...
<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-6 gap-3">
  <div>
    <h1 class="text-3xl font-bold text-white tracking-widest uppercase">Tratorval</h1>
  </div>
...
<div id="today-warning" class="hidden card p-10 mb-6 flex flex-col items-center justify-center text-center gap-4">
  ...
  <p class="text-gray-400 text-sm max-w-sm">Os dados de hoje ainda estao incompletos — o dia nao acabou.<br/>Selecione um periodo ate <strong class="text-white">{yesterday.strftime('%d/%m')}</strong> (ontem) para ver os numeros.</p>
</div>
...
<div class="card card-resumo p-5 mb-4">
  <p class="sec-title title-resumo">Resumo Geral — Todas as plataformas</p>
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
    <div>
      <p class="kpi-label">Investimento Total</p>
      <p class="kpi-val neon" id="tot-spend">—</p>
      <p class="kpi-sub">Meta + Google</p>
    </div>
    <div>
      <p class="kpi-label">Leads / Conversas</p>
      <p class="kpi-val neon" id="tot-leads">—</p>
      <p class="kpi-sub">WA + contatos Google</p>
    </div>
    <div>
      <p class="kpi-label">Custo por Lead</p>
      <p class="kpi-val text-white" id="tot-cpl">—</p>
      <p class="kpi-sub">medio geral</p>
    </div>
    <div>
      <p class="kpi-label">Alcance</p>
      <p class="kpi-val text-white" id="tot-reach">—</p>
      <p class="kpi-sub">pessoas unicas (Meta)</p>
    </div>
  </div>
</div>
...
<div class="card card-resumo p-5 mb-6">
  <h2 class="sec-title">Investimento diario + Conversas WhatsApp</h2>
  <canvas id="mainChart" height="90"></canvas>
  <p class="text-xs text-gray-700 mt-2">* Google disponivel a partir de {google_first[8:10]}/{google_first[5:7]} (linha verde empilhada).</p>
</div>

<!-- META ADS BLOCK -->
<div class="card card-meta p-5 mb-4">
  <div class="sec-title title-meta flex items-center gap-2 mb-3"><img src="https://cdn.simpleicons.org/meta/1877F2" width="18" height="18" alt="Meta"/>Meta Ads — Facebook & Instagram<span class="date-pill" id="meta-date-label"></span></div>
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
    ...
    <div><p class="kpi-label">Conversas WhatsApp</p><p class="kpi-val neon" id="m-conv">—</p><p class="kpi-sub">pessoas interessadas</p></div>
    <div><p class="kpi-label">Custo / Conversa</p><p class="kpi-val text-white" id="m-cpl">—</p><p class="kpi-sub">por pessoa interessada</p></div>
  </div>

  <div class="border-t border-gray-800 my-3" id="video-divider"></div>

  <div id="video-block">
    <p class="kpi-label mb-3">Retencao de video <span class="text-gray-600 normal-case font-normal">(disponivel a partir de 01/abr)</span></p>
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
...
<!-- META CAMPANHAS TABLE -->
<div class="card card-meta p-5 mb-4">
  <div class="sec-title title-meta flex items-center gap-2 mb-3"><img src="https://cdn.simpleicons.org/meta/1877F2" width="18" height="18" alt="Meta"/>Campanhas Meta<span class="date-pill">{camp_label}</span></div>
  <div class="overflow-x-auto">
    <table class="w-full text-sm">
      <thead><tr class="text-xs text-gray-600 border-b border-gray-800">
        ...
        <th class="text-right pb-3 pr-4">Conversas</th>
        <th class="text-right pb-3">Custo/Conversa</th>
      </tr></thead>
...
<div class="card card-google p-5 mb-4">
  <div class="sec-title title-google flex items-center gap-2 mb-3">{GADS_ICON}Google Ads — Pesquisa<span class="date-pill" id="google-date-label"></span></div>
  ...
  <div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
    ...
    <div><p class="kpi-label">CPA</p><p class="kpi-val text-white" id="g-cpa">—</p><p class="kpi-sub">custo por contato</p></div>
    <div><p class="kpi-label">ROAS</p><p class="kpi-val text-gray-600" id="g-roas">—</p><p class="kpi-sub">sem rastreamento</p></div>
    <div><p class="kpi-label">Taxa de Conv.</p><p class="kpi-val text-white" id="g-taxa">—</p><p class="kpi-sub">cliques → contato</p></div>
    <div><p class="kpi-label">Faturamento</p><p class="kpi-val text-gray-600" id="g-fat">—</p><p class="kpi-sub">sem rastreamento</p></div>
  </div>

  <div class="border-t border-gray-800 my-3"></div>

  <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
    <div><p class="kpi-label">Ticket Medio</p><p class="kpi-val text-gray-600" id="g-ticket">—</p><p class="kpi-sub">sem rastreamento</p></div>
  </div>

  <p class="text-xs text-gray-600 mt-4 italic" id="google-note"></p>
</div>

<!-- GOOGLE CAMPANHAS TABLE -->
<div class="card card-google p-5 mb-6">
  <div class="sec-title title-google flex items-center gap-2 mb-3">{GADS_ICON}Campanhas Google<span class="date-pill">{gcampmonth_label}</span></div>
  <div class="overflow-x-auto">
    <table class="w-full text-sm">
      <thead><tr class="text-xs text-gray-600 border-b border-gray-800">
        <th class="text-left pb-3 pr-4">Campanha</th>
        <th class="text-right pb-3 pr-4">Investido</th>
        <th class="text-right pb-3 pr-4">Cliques</th>
        <th class="text-right pb-3 pr-4">Contatos</th>
        <th class="text-right pb-3">Custo/contato</th>
      </tr></thead>
...
<div class="text-center text-xs text-gray-700 py-4">
  Atualizado em {today.strftime('%d/%m/%Y')}
</div>
...
function update(from,to){{
  ...
  // Google tem 1 dia de atraso — cap automático em GOOGLE_LAST
  ...
  set('m-cpm',   BRL(mCPM));
  set('m-ctr',   PCT(mCTR));
  set('m-spend', BRL(mSpend));
  set('m-clicks',mClk);
  set('m-cpc',   mClk>0?BRL(mCPC):'—');
  set('m-conv',  mConv);
  set('m-cpl',   mConv>0?BRL(mCPL):'—');
  ...
  if(hasVideo){{
    ...
    set('video-note','Dados de retencao de video disponiveis somente a partir de 01/abr. Selecione um periodo de abril pra ver.');
  }}
  ...
  set('g-cpc',    hasG&&gClicks>0?BRL(gCPC):'—');
  set('g-spend',  hasG?BRL(gSpend):'—');
  set('g-conv',   hasG?gConv:'—');
  set('g-cpa',    hasG&&gConv>0?BRL(gCPA):'—');
  set('g-roas',   '—');
  set('g-taxa',   hasG&&gClicks>0?PCT(gTaxa):'—');
  set('g-fat',    '—');
  set('g-ticket', '—');
  set('google-note', hasG
    ? 'ROAS, Faturamento e Ticket Medio indisponiveis — rastreamento de receita nao configurado.'
    : 'Sem dados Google no periodo selecionado.');

  // Resumo Geral
  const totSpend=mSpend+gSpend;
  const totLeads=mConv+gConv;
  set('tot-spend', BRL(totSpend));
  set('tot-leads', totLeads>0?totLeads:'—');
  set('tot-cpl',   totLeads>0?BRL(totSpend/totLeads):'—');
  set('tot-reach', FMT(mReach));

  // Labels de periodo
  const pLabel=fmtD(from)+' — '+fmtD(to);
  set('meta-date-label', pLabel);
  const gDateLabel = (GOOGLE_LAST && to > GOOGLE_LAST)
    ? fmtD(gEffFrom)+' — '+fmtD(GOOGLE_LAST)+' (atraso 1 dia)'
    : pLabel;
  ...
  chart=new Chart(document.getElementById('mainChart').getContext('2d'),{{
    data:{{
      ...
      datasets:[
        ...
        {{type:'line',label:'Conversas WA',data:dsConv,borderColor:'#f59e0b',backgroundColor:'rgba(245,158,11,.1)',borderWidth:2,pointRadius:allDates.length>30?2:4,tension:.3,fill:false,yAxisID:'y1'}}
      ]
    }},
    options:{{
      responsive:true,
      ...
      plugins:{{
        ...
        tooltip:{{backgroundColor:'#1a1a1a',borderColor:'#333',borderWidth:1,titleColor:'#e5e7eb',bodyColor:'#9ca3af',
          callbacks:{{label:c=>c.dataset.label==='Conversas WA'?'  Conversas: '+c.parsed.y:'  '+c.dataset.label+': R$ '+c.parsed.y.toFixed(2)}}}}
      }},
      scales:{{
  ...
  CAMPANHAS.forEach(c=>{{
    ...
    body.innerHTML+=`<tr class="border-b border-gray-800/50"><td class="py-3 pr-4"><p class="text-white font-medium">${{c.nome}}</p><p class="text-xs text-gray-500">${{c.desc}}</p></td><td class="py-3 pr-4 text-right text-gray-300">R$ ${{c.gasto.toFixed(2).replace('.',',')}}</td><td class="py-3 pr-4 text-right text-gray-300">${{c.cliques}}</td><td class="py-3 pr-4 text-right font-bold text-white">${{c.contatos}}</td><td class="py-3 text-right"><span class="${{cc}} font-semibold">${{c.cpa>0?'R$ '+c.cpa.toFixed(2).replace('.',','):'—'}}</span></td></tr>`;
    if(c.cor==='red'&&c.cpa>0)noteText='Atencao: "'+c.nome+'" com custo alto (R$ '+c.cpa.toFixed(2)+').';
  }});
  ...
  META_CAMPANHAS.forEach(c=>{{
    ...
    const cpaStr =c.conv>0?`<span class="${{cc}} font-semibold">R$ ${{c.cpa.toFixed(2).replace('.',',')}}</span>`:`<span class="text-gray-600">—</span>`;
    ...
    mbody.innerHTML+=`<tr class="border-b border-gray-800/50">
      ...
      <td class="py-3 pr-4 text-right text-gray-300">R$ ${{c.gasto.toFixed(2).replace('.',',')}}</td>
      <td class="py-3 pr-4 text-right text-gray-300">${{c.alcance.toLocaleString('pt-BR')}}</td>
...
const fp = flatpickr('#dt-range',{{
  mode:'range',
  dateFormat:'d/m/Y',
  minDate: isoToDate(MIN_DATE),
  maxDate: isoToDate(MAX_DATE),
  locale:'pt',
  showMonths: window.innerWidth>=640 ? 2 : 1,
  onOpen(sel,dateStr,instance){{
    setTimeout(()=>instance.jumpToDate(sel.length>0?sel[0]:isoToDate(MAX_DATE)),0);
  }},
  onClose(sel){{
...
# ── 7. SALVAR + ASSETS + DEPLOY ──────────────────────────────────────────────
import shutil
out_dir  = os.path.join(DEPLOY_DIR, "tratorval")
out_file = os.path.join(out_dir, "index.html")
os.makedirs(out_dir, exist_ok=True)

# Copia as logos para a pasta do deploy
...
# ── 7. DEPLOY — Cloudflare Pages via wrangler ────────────────────────────────
import subprocess

print("\nDeployando para Cloudflare Pages (wrangler)...")
env_deploy = os.environ.copy()
env_deploy["CLOUDFLARE_API_TOKEN"]  = CF_TOKEN
env_deploy["CLOUDFLARE_ACCOUNT_ID"] = CF_ACCOUNT

tratorval_dir = os.path.join(DEPLOY_DIR, "tratorval")
result = subprocess.run(
    f'npx wrangler pages deploy "{tratorval_dir}" --project-name={CF_PROJECT} --branch=main --commit-dirty=true',
    env=env_deploy, capture_output=False, shell=True, cwd=DEPLOY_DIR, timeout=90
)
if result.returncode == 0:
    print("URL: https://formuladolucro.site/tratorval/")
    print("URL alternativa: https://veritasdigital.pages.dev/tratorval/")
else:
