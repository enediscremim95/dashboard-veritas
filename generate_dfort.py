"""
Dfort Clinic Dashboard — Meta Ads apenas (EUR).
Roda: python generate_dfort.py
Conversão principal: lead (pixel Meta)
"""
...
CURRENCY_SYM  = "€"
META_CPA_BOM  = 15.0    # EUR — verde abaixo de €15/lead
META_CPA_OK   = 30.0    # EUR — amarelo abaixo de €30/lead
# ══════════════════════════════════════════════════════════════════════
...
_desc_map = {
    ...
    "DISTRIBUICAO":"Distribuicao — alcance",
    "DISTRIBUIÇÃO":"Distribuicao — alcance",
}
...
<style>
  ...
  .flatpickr-months .flatpickr-month,.flatpickr-weekdays{{background:#111!important;color:#9ca3af!important;fill:#9ca3af!important}}
...
<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-6 gap-3">
  <div>
    ...
    <p class="text-xs text-gray-600 mt-1">Clinica Estetica · Portugal</p>
  </div>
...
<div class="card card-resumo p-5 mb-4">
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
    ...
    <div><p class="kpi-label">Leads</p><p class="kpi-val neon" id="tot-leads">—</p><p class="kpi-sub">pessoas interessadas</p></div>
    <div><p class="kpi-label">CPA Medio</p><p class="kpi-val text-white" id="tot-cpa">—</p><p class="kpi-sub">por pessoa interessada</p></div>
...
<div class="card card-meta p-5 mb-6">
  <h2 class="sec-title">Investimento diario + Leads</h2>
...
<div class="card card-meta p-5 mb-4">
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
    ...
    <div><p class="kpi-label">Leads</p><p class="kpi-val neon" id="m-conv">—</p><p class="kpi-sub">pessoas interessadas</p></div>
    <div><p class="kpi-label">Custo / Lead</p><p class="kpi-val text-white" id="m-cpl">—</p><p class="kpi-sub">por pessoa interessada</p></div>
  ...
  <div class="overflow-x-auto">
    <table class="mc-table w-full text-sm">
      <thead><tr class="text-xs text-gray-600 border-b border-gray-800">
        ...
        <th class="text-right pb-3 pr-4">Leads</th>
        <th class="text-right pb-3">Custo/Lead</th>
      </tr></thead>
...
function update(from,to){{
  ...
  set('m-cpm',   EUR(mCPM));
  set('m-ctr',   PCT(mCTR));
  set('m-spend', EUR(mSpend));
  set('m-clicks',mClk);
  set('m-cpc',   mClk>0?EUR(mCPC):'—');
  set('m-conv',  mConv);
  set('m-cpl',   mConv>0?EUR(mCPL):'—');
  ...
  if(hasVideo){{
    ...
    set('video-note','Sem dados de retencao de video no periodo selecionado.');
  }}

  // Resumo
  set('tot-spend', EUR(mSpend));
  set('tot-leads', mConv > 0 ? mConv : '—');
  set('tot-cpa',   mConv > 0 ? EUR(mSpend/mConv) : '—');
  set('tot-imp',   FMT(mImp));

  // Grafico
  const labels  =mR.map(x=>fmtD(x.d));
  const dsMeta  =mR.map(x=>x.s);
  const dsConv  =mR.map(x=>x.c);
  ...
  chart=new Chart(document.getElementById('mainChart').getContext('2d'),{{
    data:{{
      labels,
      datasets:[
        {{type:'bar',label:'Meta (EUR)',data:dsMeta,backgroundColor:'rgba(59,130,246,.2)',borderColor:'#60a5fa',borderWidth:1,borderRadius:3,yAxisID:'y'}},
        {{type:'line',label:'Leads',data:dsConv,borderColor:'#f59e0b',backgroundColor:'rgba(245,158,11,.1)',borderWidth:2,pointRadius:mR.length>30?2:4,tension:.3,fill:false,yAxisID:'y1'}}
      ]
    }},
    options:{{
      responsive:true,
      ...
      plugins:{{
        ...
        tooltip:{{backgroundColor:'#1a1a1a',borderColor:'#333',borderWidth:1,titleColor:'#e5e7eb',bodyColor:'#9ca3af',
          callbacks:{{label:c=>c.dataset.label==='Leads'?'  Leads: '+c.parsed.y:'  '+c.dataset.label+': EUR '+c.parsed.y.toFixed(2)}}}}
      }},
      scales:{{
        x:{{grid:{{color:'#1a1a1a'}},ticks:{{color:'#6b7280',font:{{size:10}},maxRotation:45}}}},
        y:{{grid:{{color:'#1a1a1a'}},ticks:{{color:'#6b7280',font:{{size:10}},callback:v=>'EUR '+v}}}},
  ...
  camps.forEach(c=>{{
    ...
    const cpaStr =c.conv>0?`<span class="${{cc}} font-semibold">EUR ${{c.cpa.toFixed(2)}}</span>`:`<span class="text-gray-600">—</span>`;
    ...
    mbody.innerHTML+=`<tr ${{rowClick}}>
      ...
      <td class="py-3 pr-4 text-right text-gray-300">EUR ${{c.gasto.toFixed(2)}}</td>
...
const fp = flatpickr('#dt-range',{{
  ...
  minDate: isoToDate(MIN_DATE), maxDate: isoToDate(MAX_DATE),
  locale:'pt', showMonths: window.innerWidth>=640 ? 2 : 1,
  onOpen(sel,_,instance){{setTimeout(()=>instance.jumpToDate(sel.length>0?sel[0]:isoToDate(MAX_DATE)),0);}},
  onClose(sel){{
...
env_d["CLOUDFLARE_API_TOKEN"]  = CF_TOKEN
env_d["CLOUDFLARE_ACCOUNT_ID"] = CF_ACCOUNT

result = subprocess.run(
    f'npx wrangler pages deploy "{dist_dir}" --project-name={CF_PROJECT} --branch=main --commit-dirty=true',
    env=env_d, capture_output=False, shell=True, cwd=DEPLOY_DIR, timeout=120
)
