"""
BIVAR Clinic Dashboard — geração + deploy automático.
Roda: python generate_bivar.py

BÍVAR Clinic · Portugal
Conversão: offsite_conversion.fb_pixel_lead (Leads)
Moeda: EUR (euro)
NOTA: conta Meta intencionalmente pausada (sem criativos ainda) — zeros no Meta são normais.
"""
...
#  CONFIG
# ══════════════════════════════════════════════════════════════════════
CLIENT_NAME   = "BÍVAR Clinic"
CLIENTE_NOME  = CLIENT_NAME
CLIENTE_SUB   = "Clinica · Portugal"
CF_SUBFOLDER  = "bivar"
...
META_ACT      = "act_478439174769598"
SHEET_ID      = "1795oDIVuSKLjniXzPrFbUr_vNDHqPm-OsoBp8_VJWkI"
CLIENTE_SLUG  = "BIVAR"

CONV_ACTION   = "offsite_conversion.fb_pixel_lead"
CONV_LABEL    = "Leads"
CONV_UNIT     = "leads captados"
CONV_CPL_LBL  = "Custo por Lead"
CONV_CPL_UNIT = "lead"
...
meta_first   = META[0][0] if META else ""

if META and GOOGLE:
    min_date = min(meta_first, google_first)
elif GOOGLE:
    min_date = google_first
elif META:
    min_date = meta_first
else:
    min_date = since_str
...
<div class="card card-resumo p-5 mb-4">
  <p class="sec-title title-resumo">Resumo Geral &mdash; Meta + Google</p>
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
    <div><p class="kpi-label">Investimento Total</p><p class="kpi-val neon" id="tot-spend">&mdash;</p><p class="kpi-sub">Meta + Google</p></div>
    <div><p class="kpi-label">{CONV_LABEL}</p><p class="kpi-val neon" id="tot-leads">&mdash;</p><p class="kpi-sub">{CONV_UNIT} (Meta + Google)</p></div>
    <div><p class="kpi-label">CPA Medio</p><p class="kpi-val text-white" id="tot-cpa">&mdash;</p><p class="kpi-sub">custo por lead</p></div>
    <div><p class="kpi-label">Impressoes</p><p class="kpi-val text-white" id="tot-imp">&mdash;</p><p class="kpi-sub">Meta + Google</p></div>
  </div>
</div>
...
<div class="card card-resumo p-5 mb-6">
  ...
  <p class="text-xs text-gray-700 mt-2">* Google disponivel a partir de {_google_first_fmt}. Dados do Google com 1 dia de atraso. Meta pausada — zeros sao esperados.</p>
</div>
...
<div class="card card-meta p-5 mb-4">
  <div class="sec-title title-meta flex items-center gap-2 mb-3">
    ...
    Meta Ads &mdash; Facebook &amp; Instagram
    <span class="date-pill" id="meta-date-label"></span>
  </div>
  <div class="grid grid-cols-2 md:grid-cols-6 gap-3 mb-4">
    <div><p class="kpi-label">Alcance</p><p class="kpi-val text-white" id="m-reach">&mdash;</p><p class="kpi-sub">pessoas unicas</p></div>
    <div><p class="kpi-label">Impressoes</p><p class="kpi-val text-white" id="m-imp">&mdash;</p><p class="kpi-sub">exibicoes totais</p></div>
    <div><p class="kpi-label">Frequencia</p><p class="kpi-val text-white" id="m-freq">&mdash;</p><p class="kpi-sub">media por pessoa</p></div>
    <div><p class="kpi-label">CPM</p><p class="kpi-val text-white" id="m-cpm">&mdash;</p><p class="kpi-sub">custo por 1.000</p></div>
    <div><p class="kpi-label">CTR</p><p class="kpi-val text-white" id="m-ctr">&mdash;</p><p class="kpi-sub">taxa de clique</p></div>
    <div><p class="kpi-label">Investimento</p><p class="kpi-val neon" id="m-spend">&mdash;</p><p class="kpi-sub">total no periodo</p></div>
  </div>
  <div class="border-t border-gray-800 my-3"></div>
  <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
    <div><p class="kpi-label">Cliques</p><p class="kpi-val text-white" id="m-clicks">&mdash;</p><p class="kpi-sub">em links</p></div>
    <div><p class="kpi-label">CPC</p><p class="kpi-val text-white" id="m-cpc">&mdash;</p><p class="kpi-sub">custo por clique</p></div>
    <div><p class="kpi-label">{CONV_LABEL}</p><p class="kpi-val neon" id="m-conv">&mdash;</p><p class="kpi-sub">{CONV_UNIT}</p></div>
    <div><p class="kpi-label">{CONV_CPL_LBL}</p><p class="kpi-val text-white" id="m-cpl">&mdash;</p><p class="kpi-sub">{CONV_CPL_UNIT}</p></div>
    <div><p class="kpi-label">Conversas (Wpp + DM)</p><p class="kpi-val text-white" id="m-msg">&mdash;</p><p class="kpi-sub">WhatsApp + Instagram Direct</p></div>
  </div>
  <div class="border-t border-gray-800 my-3" id="video-divider"></div>
  <div id="video-block">
    <p class="kpi-label mb-3">Retencao de video</p>
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <div><div class="flex justify-between mb-1"><span class="text-xs text-gray-400">View 25%</span><span class="text-xs text-white" id="m-v25">&mdash;</span></div><div class="video-bar-bg"><div class="video-bar" id="bar-v25" style="width:0%"></div></div></div>
      <div><div class="flex justify-between mb-1"><span class="text-xs text-gray-400">View 50%</span><span class="text-xs text-white" id="m-v50">&mdash;</span></div><div class="video-bar-bg"><div class="video-bar" id="bar-v50" style="width:0%;background:#60a5fa"></div></div></div>
      <div><div class="flex justify-between mb-1"><span class="text-xs text-gray-400">View 75%</span><span class="text-xs text-white" id="m-v75">&mdash;</span></div><div class="video-bar-bg"><div class="video-bar" id="bar-v75" style="width:0%;background:#fbbf24"></div></div></div>
      <div><div class="flex justify-between mb-1"><span class="text-xs text-gray-400">View 95%</span><span class="text-xs text-white" id="m-v95">&mdash;</span></div><div class="video-bar-bg"><div class="video-bar" id="bar-v95" style="width:0%;background:#f87171"></div></div></div>
    </div>
...
<div class="card card-google p-5 mb-4">
  <div class="sec-title title-google flex items-center gap-2 mb-3">
    {GADS_ICON}Google Ads &mdash; Pesquisa
    <span class="date-pill" id="google-date-label"></span>
  </div>
  <div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
    <div><p class="kpi-label">Impressoes</p><p class="kpi-val text-white" id="g-imp">&mdash;</p><p class="kpi-sub">vezes exibido</p></div>
    <div><p class="kpi-label">CTR</p><p class="kpi-val text-white" id="g-ctr">&mdash;</p><p class="kpi-sub">taxa de clique</p></div>
    <div><p class="kpi-label">Cliques</p><p class="kpi-val text-white" id="g-clicks">&mdash;</p><p class="kpi-sub">visitas ao site</p></div>
    <div><p class="kpi-label">CPC</p><p class="kpi-val text-white" id="g-cpc">&mdash;</p><p class="kpi-sub">custo por clique</p></div>
    <div><p class="kpi-label">Investimento</p><p class="kpi-val neon" id="g-spend">&mdash;</p><p class="kpi-sub">total no periodo</p></div>
  </div>
  <div class="border-t border-gray-800 my-3"></div>
  <div class="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
    <div><p class="kpi-label">Conversoes</p><p class="kpi-val neon" id="g-conv">&mdash;</p><p class="kpi-sub">{CONV_UNIT}</p></div>
    <div><p class="kpi-label">CPA</p><p class="kpi-val text-white" id="g-cpa">&mdash;</p><p class="kpi-sub">custo por lead</p></div>
    <div><p class="kpi-label">Taxa de Conv.</p><p class="kpi-val text-white" id="g-taxa">&mdash;</p><p class="kpi-sub">cliques → lead</p></div>
  </div>
...
let chart = null;
const CUR    = n => '{CURRENCY_SYM} ' + n.toFixed(2);
...
function update(from,to){{
  ...
  set('google-note', hasG ? '' : 'Sem dados Google no periodo selecionado (campanha pausada ou sem gasto).');
  ...
  chart=new Chart(document.getElementById('mainChart').getContext('2d'),{{
    data:{{
      labels,
      datasets:[
        {{type:'bar',label:'Meta ({CURRENCY_SYM})',data:dsMeta,backgroundColor:'rgba(59,130,246,.2)',borderColor:'#60a5fa',borderWidth:1,borderRadius:3,stack:'sp',yAxisID:'y'}},
        {{type:'bar',label:'Google ({CURRENCY_SYM})',data:dsGoogle,backgroundColor:'rgba(0,255,136,.18)',borderColor:'#00ff88',borderWidth:1,borderRadius:3,stack:'sp',yAxisID:'y'}},
    ...
    options:{{
      ...
      plugins:{{
        ...
        tooltip:{{backgroundColor:'#1a1a1a',borderColor:'#333',borderWidth:1,titleColor:'#e5e7eb',bodyColor:'#9ca3af',
          callbacks:{{label:c=>c.dataset.label==='{CONV_LABEL}'?'  {CONV_LABEL}: '+c.parsed.y:'  '+c.dataset.label+': {CURRENCY_SYM} '+c.parsed.y.toFixed(2)}}}}
      }},
      scales:{{
        x:{{stacked:true,grid:{{color:'#1a1a1a'}},ticks:{{color:'#6b7280',font:{{size:10}},maxRotation:45}}}},
        y:{{stacked:true,grid:{{color:'#1a1a1a'}},ticks:{{color:'#6b7280',font:{{size:10}},callback:v=>'{CURRENCY_SYM} '+v}}}},
  ...
  gcamps.forEach(c=>{{
    ...
    body.innerHTML+=`<tr class="border-b border-gray-800/50">
      ...
      <td class="py-3 text-right"><span class="${{cc}} font-semibold">${{cpa>0?'€ '+cpa.toFixed(2):'—'}}</span></td>
    </tr>`;
  ...
  mbody.innerHTML='';
  if(camps.length===0){{
    mbody.innerHTML='<tr><td colspan="7" class="py-4 text-center text-gray-600 text-xs">Meta pausada — sem campanhas com gasto no periodo.</td></tr>';
  }}
  camps.forEach(c=>{{
    const cc=c.cor==='green'?'tag-green':c.cor==='blue'?'text-blue-400':c.cor==='yellow'?'tag-yellow':'tag-red';
    const cpaStr=c.conv>0?'<span class="'+cc+' font-semibold">{CURRENCY_SYM} '+c.cpa.toFixed(2)+'</span>':'<span class="text-gray-600">--</span>';
    ...
    mbody.innerHTML+='<tr '+rowClick+'><td class="py-3 pl-1 pr-2 w-6">'+arrowEl+'</td><td class="py-3 pr-4"><p class="text-white font-medium">'+c.nome+'</p><p class="text-xs text-gray-500">'+c.desc+'</p></td><td class="py-3 pr-4 text-right text-gray-300">{CURRENCY_SYM} '+c.gasto.toFixed(2)+'</td><td class="py-3 pr-4 text-right text-gray-300">'+c.reach.toLocaleString('pt-PT')+'</td><td class="py-3 pr-4 text-right text-gray-300">'+c.lk+'</td><td class="py-3 pr-4 text-right">'+convStr+'</td><td class="py-3 text-right">'+cpaStr+'</td></tr><tr id="drill-'+c.id+'" class="drill-row hidden"></tr>';
  }});
  if(camps.length>=1){{
    ...
    mbody.innerHTML+=`<tr class="border-t-2 border-gray-600 bg-white/[.02]">
      ...
      <td class="py-3 pr-4 text-right font-bold text-white">{CURRENCY_SYM} ${{mt.g.toFixed(2)}}</td>
      ...
      <td class="py-3 text-right font-bold text-white">${{mcpa>0?'{CURRENCY_SYM} '+mcpa.toFixed(2):'—'}}</td>
    </tr>`;
  }}
}}
