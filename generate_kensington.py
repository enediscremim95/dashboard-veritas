"""
Kensington Olympia Dashboard — geração + deploy automático.
Roda: python generate_kensington.py

Estetica · Londres
Meta + Google. GBP. Conversão: link_click.
HAS_ROAS = False. Google usa fallback Visao Geral (script antigo sem Tendencia Diaria).
"""
...
#  CONFIG
# ══════════════════════════════════════════════════════════════════════
CLIENTE_NOME   = "Kensington Olympia"
CLIENTE_SUB    = "Estetica · Londres"
CF_SUBFOLDER   = "kensington"

HAS_GOOGLE     = True
HAS_ROAS       = False

META_ACT       = "act_2228189290901209"
SHEET_ID       = "1A9slrRovmPYoswUXvsYmmyQR18YwJCVB0I9JfBsgDs0"
CLIENTE_SLUG   = "Kensington"

CONV_ACTION    = "contact"
CONV_LABEL     = "Contatos"
CONV_UNIT      = "cliques no WhatsApp"
CONV_CPL_LBL   = "Custo / Contato"
CONV_CPL_UNIT  = "por contato"
...
_desc_map = {
    ...
    "CONVERSAO":    "Distribuicao — alcance para novo publico",
    "CONVERSÃO":    "Distribuicao — alcance para novo publico",
    "DISTRIBUICAO": "Distribuicao — alcance para novo publico",
    "DISTRIBUIÇÃO": "Distribuicao — alcance para novo publico",
    "AWARENESS":    "Awareness — alcance amplo",
}
...
def fix_dict(d):
    if isinstance(d, dict): return {fix_encoding(k): fix_dict(v) for k, v in d.items()}
...
_WINDSOR_CONV = {
    ...
    "landing_page_view": "actions_landing_page_view",
    "contact":           "actions_offsite_conversion_fb_pixel_contact",
}
...
for row in meta_rows:
    ...
    if cid not in CAMP_META_INFO:
        desc = next((v for k, v in _desc_map.items() if cid and cid and k in cid.upper()), "Campanha ativa")
...
# AD_DRILL — nível de anuncio via ad_rows
...
for row in ad_rows:
    ...
    if cn not in AD_DRILL:
        AD_DRILL[cn] = {}
    if an not in AD_DRILL[cn]:
        AD_DRILL[cn][an] = {"name": an, "spend": 0, "conv": 0, "ads": {}}
    if adn not in AD_DRILL[cn][an]["ads"]:
        AD_DRILL[cn][an]["ads"][adn] = {"name": adn, "spend": 0, "conv": 0, "thumb": thumb, "link": link}
    AD_DRILL[cn][an]["spend"] += s
    AD_DRILL[cn][an]["conv"]  += c
    AD_DRILL[cn][an]["ads"][adn]["spend"] += s
    AD_DRILL[cn][an]["ads"][adn]["conv"]  += c
    if not AD_DRILL[cn][an]["ads"][adn]["thumb"] and thumb:
        AD_DRILL[cn][an]["ads"][adn]["thumb"] = thumb
    if not AD_DRILL[cn][an]["ads"][adn]["link"] and link:
        AD_DRILL[cn][an]["ads"][adn]["link"] = link
...
# CONV_ACTION == "link_click": conv e lk são iguais
...
<div class="card card-resumo p-5 mb-6">
  ...
  <p class="text-xs text-gray-700 mt-2">* Google: dados diarios indisponiveis (script desatualizado). Google exibido no bloco abaixo como resumo 30d.</p>
</div>
...
// Renderiza campanhas Google (estaticas — dados da planilha)
(function renderGoogleCamps(){{
  const tbody=document.getElementById('camp-body');
  if(!tbody)return;
  if(!CAMPANHAS||CAMPANHAS.length===0){{
    tbody.innerHTML='<tr><td colspan="5" class="py-4 text-center text-gray-600">Sem campanhas ativas no periodo</td></tr>';
    return;
  }}
  let noteText='';
  CAMPANHAS.forEach(c=>{{
    ...
    tbody.innerHTML+=`<tr class="border-b border-gray-800/50">
      ...
      <td class="py-3 pr-4 text-right font-bold text-white">${{c.contatos>0?c.contatos:'—'}}</td>
  ...
  if(CAMPANHAS.length>=1){{
    const gt=CAMPANHAS.reduce((a,c)=>{{a.g+=c.gasto;a.cl+=c.cliques;a.cv+=c.contatos;return a;}},{{g:0,cl:0,cv:0}});
    const gcpa=gt.cv>0?gt.g/gt.cv:0;
    tbody.innerHTML+=`<tr class="border-t-2 border-gray-600 bg-white/[.02]">
      <td class="py-2 pl-1 text-xs text-gray-400 font-semibold uppercase tracking-wide">Total</td>
      <td class="py-2 pr-4 text-right text-gray-200 font-semibold">£ ${{gt.g.toFixed(2)}}</td>
      <td class="py-2 pr-4 text-right text-gray-200 font-semibold">${{gt.cl}}</td>
      <td class="py-2 pr-4 text-right text-white font-bold">${{gt.cv>0?gt.cv:'—'}}</td>
      <td class="py-2 text-right text-gray-200 font-semibold">${{gcpa>0?'£ '+gcpa.toFixed(2):'—'}}</td>
    </tr>`;
  }}
...
function update(from,to){{
  ...
  // Google totais — com fallback quando Tendencia Diaria vazia
  let gSpend,gImp,gClicks,gConv,usedFallback=false;
  if(GOOGLE_DATA.length>0&&gR.length>0){{
  ...
  let gNote='';
  if(usedFallback) gNote='Dados Google referentes aos ultimos 30 dias (script do Google Ads desatualizado — aguardando atualizacao).';
  else if(!hasG) gNote='Sem dados Google no periodo selecionado.';
  ...
  // Resumo Geral — usa fallback Google quando disponivel
  ...
  // Grafico (Meta stacked + Google stacked + linha conv)
  ...
  const labels  =allDates.length>0?allDates.map(fmtD):mR.map(x=>fmtD(x.d));
  const dataSrc =allDates.length>0?allDates:mR.map(x=>x.d);
  const dsMeta  =dataSrc.map(d=>mByD[d]?mByD[d].s:0);
  const dsGoogle=dataSrc.map(d=>gByD[d]?gByD[d].s:0);
  const dsConv  =dataSrc.map(d=>mByD[d]?mByD[d].c:0);
  ...
  chart=new Chart(document.getElementById('mainChart').getContext('2d'),{{
    data:{{
      ...
      datasets:[
        ...
        {{type:'line',label:'{CONV_LABEL}',data:dsConv,borderColor:'#f59e0b',backgroundColor:'rgba(245,158,11,.1)',borderWidth:2,pointRadius:labels.length>30?2:4,tension:.3,fill:false,yAxisID:'y1'}}
      ]
    }},
    options:{{
      responsive:true,
  ...
  camps.forEach(c=>{{
    ...
    mbody.innerHTML+=`<tr ${{rowClick}}>
      ...
      <td class="py-3 pr-4"><p class="text-white font-medium">${{c.nome}}</p><p class="text-xs text-gray-500">${{c.desc}}</p></td>
      <td class="py-3 pr-4 text-right text-gray-300">£ ${{c.gasto.toFixed(2)}}</td>
      <td class="py-3 pr-4 text-right text-gray-300">${{c.reach.toLocaleString('en-GB')}}</td>
  ...
  if(camps.length===0)mbody.innerHTML='<tr><td colspan="7" class="py-4 text-center text-gray-600">Sem campanhas com gasto no periodo</td></tr>';
  if(camps.length>=1){{
    ...
    mbody.innerHTML+=`<tr class="border-t-2 border-gray-600 bg-white/[.02]">
      ...
      <td class="py-2 pr-4 text-right text-gray-200 font-semibold">${{mt.r.toLocaleString('en-GB')}}</td>
      <td class="py-2 pr-4 text-right text-gray-200 font-semibold">${{mt.lk}}</td>
      <td class="py-2 pr-4 text-right text-white font-bold">${{mt.cv}}</td>
