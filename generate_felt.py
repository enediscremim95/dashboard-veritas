"""
Felt Imobiliaria Dashboard — geracao + deploy automatico.
Roda: python generate_felt.py

Felt Imobiliaria · Duas contas Meta (CA02 Alphaville + CA03 COD)
Conversao: onsite_conversion.total_messaging_connection (Contatos WhatsApp)
Moeda: BRL (R$)
"""
...
#  CONFIG
# ══════════════════════════════════════════════════════════════════════
CLIENT_NAME   = "Felt Imobiliaria"
CLIENTE_NOME  = "Felt Imobiliária"
CLIENTE_SUB   = "Felt Imobiliária · Duas contas Meta"
CF_SUBFOLDER  = "felt"
CURRENCY_SYM  = "R$"
...
CONV_LABEL    = "Contatos WhatsApp"
CONV_UNIT     = "contatos"
CONV_CPL_LBL  = "Custo por Contato"
CONV_CPL_UNIT = "contato"

META_CPA_OK   = 200   # abaixo = bom
META_CPA_WARN = 400   # acima = critico

# Duas contas Meta
META_ACT_CA02  = "act_1124262868759849"   # CA02 Alphaville ~R$11.075/mes
META_ACT_CA03  = "act_1415679453619304"   # CA03 COD (conta nova)
...
_desc_map = {
    ...
    "ALPHAVILLE":  "Alphaville — empreendimento principal",
    "COD":         "COD — empreendimento COD",
    "LANCAMENTO":  "Lancamento — novo produto",
    "LANÇAMENTO":  "Lancamento — novo produto",
    "IMOVEL":      "Imovel especifico",
    "IMÓVEL":      "Imovel especifico",
}
...
# Felt tem 2 contas; todo dado vem do CA02 (Alphaville). CA03 vazio.
meta_js_ca02 = meta_js
meta_js_ca03 = []
camp_js_ca02 = CAMP_META_INFO
camp_js_ca03 = {}
AD_DRILL_CA02 = {}
AD_DRILL_CA03 = {}
...
<style>
  ...
  .period-btn{{padding:6px 14px;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;border:1px solid #2a2a2a;background:#161616;color:#9ca3af;transition:all .15s}}
  .period-btn:hover{{border-color:#444;color:#e5e7eb}}
  .period-btn.active{{background:rgba(0,255,136,.12);border-color:rgba(0,255,136,.4);color:#00ff88}}
  ...
  .acc-switcher{{display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap}}
  .acc-tag{{font-size:10px;color:#6b7280;background:rgba(30,30,30,.8);border:1px solid #2a2a2a;border-radius:4px;padding:2px 8px;margin-left:8px;font-weight:500;text-transform:none;letter-spacing:normal}}
...
<div class="card p-4 mb-6">
  <div class="flex flex-wrap items-center gap-2">
    ...
    <div class="flex flex-wrap gap-1">
      <button class="period-btn" onclick="setPeriod('month')" id="btn-month">Este mes</button>
      <button class="period-btn" onclick="setPeriod('7')"     id="btn-7">7 dias</button>
      <button class="period-btn" onclick="setPeriod('14')"    id="btn-14">14 dias</button>
      <button class="period-btn" onclick="setPeriod('30')"    id="btn-30">30 dias</button>
      <button class="period-btn" onclick="setPeriod('90')"    id="btn-90">90 dias</button>
    </div>
...
<!-- META ADS — com switcher de conta -->
<div class="card card-meta p-5 mb-4">
  ...
  <div class="acc-switcher">
    <button id="btn-ca02" class="period-btn active" onclick="switchAccount('ca02')">🏘️ Alphaville (CA02)</button>
    <button id="btn-ca03" class="period-btn" onclick="switchAccount('ca03')">🏢 COD (CA03)</button>
    <span id="acc-id-tag" class="acc-tag">act_1124262868759849</span>
  </div>
...
<div class="card card-meta p-5 mb-6">
  <div class="sec-title title-meta flex items-center gap-2 mb-3">
    ...
    <span id="camp-acc-tag" class="acc-tag"></span>
  </div>
...
<script>
// Dados das duas contas
const META_DATA_CA02 = {json.dumps(meta_js_ca02, separators=(',', ':'))};
const META_DATA_CA03 = {json.dumps(meta_js_ca03, separators=(',', ':'))};
const CAMP_CA02      = {json.dumps(camp_js_ca02, separators=(',', ':'))};
const CAMP_CA03      = {json.dumps(camp_js_ca03, separators=(',', ':'))};
const AD_DRILL_CA02  = {json.dumps(AD_DRILL_CA02, separators=(',', ':'))};
const AD_DRILL_CA03  = {json.dumps(AD_DRILL_CA03, separators=(',', ':'))};

const META_CPA_OK_JS   = {META_CPA_OK};
const META_CPA_WARN_JS = {META_CPA_WARN};
const META_CPA_BOM_JS  = {META_CPA_OK};   // alias para drill

const ACC_IDS = {{ca02: '{META_ACT_CA02}', ca03: '{META_ACT_CA03}'}};
const ACC_NOMES = {{ca02: 'Alphaville (CA02)', ca03: 'COD (CA03)'}};

let activeAccount = 'ca02';
let currentFrom   = '';
let currentTo     = '';
let chart         = null;
...
function switchAccount(key) {{
  activeAccount = key;
  // Atualizar visual dos botoes de conta
  ['ca02','ca03'].forEach(k => {{
    const btn = document.getElementById('btn-' + k);
    if(btn) btn.classList.toggle('active', k === key);
  }});
  // Atualizar tag com ID da conta
  const tagEl = document.getElementById('acc-id-tag');
  if(tagEl) tagEl.textContent = ACC_IDS[key];
  const campTagEl = document.getElementById('camp-acc-tag');
  if(campTagEl) campTagEl.textContent = ACC_NOMES[key];
  // Limpar drill rows ja renderizados (dados mudam de conta)
  document.querySelectorAll('[id^="drill-"]').forEach(el => {{
  ...
  // Re-renderizar com mesmo periodo
  if(currentFrom && currentTo) update(currentFrom, currentTo);
}}
...
function renderDrill(campId, container) {{
  const adDrill = activeAccount === 'ca02' ? AD_DRILL_CA02 : AD_DRILL_CA03;
  const data = adDrill[campId];
  ...
  Object.values(data).sort((a,b)=>b.spend-a.spend).forEach(aset => {{
    ...
    html += `<div class="adset-hdr"><span class="text-xs font-bold text-blue-300 uppercase tracking-wide">${{aset.name}}</span><span class="text-xs text-gray-500 ml-3">R$ ${{aset.spend.toFixed(2)}} · ${{aset.conv}} contato${{aset.conv!==1?'s':'.'}}${{asetCpa>0?' · R$ '+asetCpa.toFixed(2)+'/contato':''}}</span></div>`;
    html += '<div class="space-y-2 mb-4">';
    aset.ads.forEach(ad => {{
      const adCpa = ad.conv>0 ? ad.spend/ad.conv : 0;
      const cpaCls = ad.conv>0&&adCpa<META_CPA_OK_JS?'text-green-400':ad.conv>0&&adCpa<META_CPA_WARN_JS?'text-yellow-400':ad.conv>0?'text-red-400':'text-gray-600';
      ...
      const previewBtn = ad.preview ? `<a href="${{ad.preview}}" target="_blank" rel="noopener" class="ml-2 text-gray-500 hover:text-blue-400 transition-colors" title="Ver anuncio ao vivo">👁️</a>` : '';
      html += `<div class="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 transition-colors">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs text-gray-500 mt-0.5">R$ ${{ad.spend.toFixed(2)}} · <span class="${{cpaCls}} font-semibold">${{ad.conv}} contato${{ad.conv!==1?'s.':'.'}}</span>${{adCpa>0?' · R$ '+adCpa.toFixed(2)+'/contato':''}}</p>
        </div>
        ${{previewBtn}}
      </div>`;
    }});
...
function update(from, to) {{
  currentFrom = from;
  currentTo   = to;
  // Selecionar dados da conta ativa
  const metaData = activeAccount === 'ca02' ? META_DATA_CA02 : META_DATA_CA03;
  const campData = activeAccount === 'ca02' ? CAMP_CA02      : CAMP_CA03;
  const adDrill  = activeAccount === 'ca02' ? AD_DRILL_CA02  : AD_DRILL_CA03;

  const mR = metaData.filter(x => x.d >= from && x.d <= to);
  ...
  chart = new Chart(document.getElementById('mainChart').getContext('2d'), {{
    ...
    options: {{
      ...
      scales: {{
        ...
        y1: {{position:'right', grid:{{drawOnChartArea:false}}, ticks:{{color:'#fbbf24', font:{{size:10}}, callback:v=>v+' contatos'}}}}
      }}
    }}
  }});

  // Campanhas — agregar por periodo e conta ativa
  set('meta-camp-date-label', fmtD(from)+' — '+fmtD(to));
  const campInfo  = campData.info;
  const campDaily = campData.daily;
  const byId = {{}};
  campDaily.filter(x => x.d>=from && x.d<=to).forEach(row => {{
  ...
  const camps = Object.entries(byId)
    ...
    .map(([id,c]) => {{
      const info = campInfo[id] || {{nome:id, desc:'—'}};
      const cpa  = c.conv > 0 ? c.gasto/c.conv : 0;
      const cor  = c.conv>0&&cpa<{META_CPA_OK}?'green':c.conv>0&&cpa<{META_CPA_WARN}?'yellow':c.conv>0?'red':'blue';
  ...
  camps.forEach(c => {{
    ...
    const hasDrill  = adDrill[c.id] && Object.keys(adDrill[c.id]).length > 0;
...
function clearActive() {{
  ['month','7','14','30','90'].forEach(id => document.getElementById('btn-'+id).classList.remove('active'));
}}
...
// Inicializar tag da conta padrao
document.getElementById('acc-id-tag').textContent = ACC_IDS[activeAccount];
document.getElementById('camp-acc-tag').textContent = ACC_NOMES[activeAccount];
