"""
Kooldent Dashboard — geração + deploy automático.
Roda: python generate_kooldent.py

Clínica Dentária · Portugal
Conversão: lead
Moeda: EUR (€)
"""
...
#  CONFIG
# ══════════════════════════════════════════════════════════════════════
CLIENTE_NOME  = "Kooldent"
CLIENTE_SUB   = "Clinica Dentaria · Portugal"
CF_SUBFOLDER  = "kooldent"

HAS_GOOGLE    = False
HAS_ROAS      = False

META_ACT      = "act_1037155311543762"
CLIENTE_SLUG  = "Kooldent"
...
function renderDrill(campId, container) {{
  ...
  const CUR_D = n => '{CURRENCY_SYM} ' + n.toFixed(2);
...
function update(from,to){{
  ...
  chart=new Chart(document.getElementById('mainChart').getContext('2d'),{{
    data:{{
      ...
      datasets:[
        {{type:'bar',label:'Meta ({CURRENCY_SYM})',data:allDates.map(d=>{{const r=mR.find(x=>x.d===d);return r?r.s:0;}}),backgroundColor:'rgba(59,130,246,.2)',borderColor:'#60a5fa',borderWidth:1,borderRadius:3,yAxisID:'y'}},
    ...
    options:{{
      ...
      scales:{{
        ...
        y:{{grid:{{color:'#1a1a1a'}},ticks:{{color:'#6b7280',font:{{size:10}},callback:v=>'{CURRENCY_SYM} '+v}}}},
